...
var argv = require("yargs")
  ...
  .describe("debug", "Print stack traces on error")
  .alias("debug", "d")
  .describe("watch", "Watch the source for changes and reload")
  .describe(
    "whitelist",
    "Whitelist a root directory where the javascript files can be"
  )
  .describe("webpack", "Path to webpack config for isomorphic rendering")
  .default("whitelist", process.env.REACT_WHITELIST)
  .help("h")
  .alias("h", "help").argv;

morgan.token("file", function (req, res) {
  return path.basename(req.body.path_to_source);
});
...
app.use(
  morgan(
    "[:date[iso]] :method :url :status :response-time ms - :file :res[content-length]"
  )
);

if (argv.webpack) {
  var pathToWebpackConfig = argv.webpack;
  var IsomorphicWebpack = require("isomorphic-webpack");
  var createIsomorphicWebpack = IsomorphicWebpack.createIsomorphicWebpack;
...
var Component = function Component(pathToSource) {
  ...
  if (
    ...
    throw new Error(
      this.component.name +
        " is not a valid React component because it has no render method: " +
        this.pathToSource
    );
  }
  ...
  if (argv.watch) {
    var watcher = fs.watch(this.pathToSource, function reloader(
      event,
      filename
    ) {
